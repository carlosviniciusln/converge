...
export class PlanejamentoCadastroV2Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log('NU PLANEJAMENTO ORÇAMENTO', this.nuPlanejamentoOrcamento);
    if (this.nuPlanejamentoOrcamento == null) {
      this.nuPlanejamentoOrcamento = this.nuPlanejamento?.nuPlanejamento;
    }

    const nU_ORC = this.nuPlanejamento?.nuOrc;
    ...
    if (
      this.nuPlanejamento?.coFilial == this.currentProfile.coUnidade ||
  ...
  editarTextos() {
    ...
    this.subTitulo =
      this.currentPageAction == PageAction.Cadastrar
        ? element.subTitle
        : `${element.subTitle} ${this.nuPlanejamento?.nuOrc}`;
  ...
  try {
    ...
    >(
      `v1/PlanejamentoOrcamentarioV/detalhar-itens-planejados-novo`, {
        nuOrc: this.nuPlanejamento?.nuOrc,
        nuPlanejamento: this.nuPlanejamento?.nuPlanejamento
      }
    );
    ...
    // ===== Dados principais =====
    this.form.patchValue({
      nuPlanejamentoOrcamentario: planejamento.nuPlanejamento,
...
private criarFormGroupPrevisao(
  ...
  public async obterPlanejamentoItemHistorico(): Promise<void> {
    try {
      ...
      this.filtroRegistros.NuPlanejamento =this.nuPlanejamento?.nuPlanejamento;
      this.filtroRegistros.NuTipoDemanda = this.nuPlanejamento?.nuTipoDemanda;
      this.filtroRegistros.NuOrc = this.nuPlanejamento.nuOrc;
  ...
  public async Alterar(): Promise<void> {
    try {
      ...
      if (
        ...
        if (
          this.nuPlanejamento.nuStatusPlanejamento == 5 &&
        ...
        if (
          this.nuPlanejamento.nuStatusPlanejamento == 7 &&
        ...
        if (
          this.nuPlanejamento.nuStatusPlanejamento == 9 &&
      ...
      if (
        this.nuPlanejamento.nuStatusPlanejamento == 4 && //cancelado
  ...
  async Excluir(planejamentoOrcamentario: Gcpvw055DetalheTelaConsultaV2DTO) {
    const alert = await Swal.fire({
      title: '',
      text: `Deseja realmente excluir Planejamento Orçamentário Cód: ${this.nuPlanejamento?.nuOrc}?`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Sim, deletar!',
      cancelButtonText: 'Não, cancelar!',
