...
public async onSalvarMudancasStatus(): Promise<void> {
    ...
    try {
      ...
      const itensSelecionados = this.perfilOrcamento || this.perfilAdm ? this.planejamentos.contratos.filter(p => p.stSelecionado): this.planejamentos.contratos.filter(p => p.stSelecionado && this.perfilUnidade == p.coFilial);
  ...
  selecionarTodosItens() {
   ...
   atualizarStatusSelecionados() {
    const itensNaoSelecionados = this.planejamentos.contratos.filter(p => p.stSelecionado == false)
    ...
    const itensSelecionados = this.perfilOrcamento || this.perfilAdm ? this.planejamentos.contratos.filter(p => p.stSelecionado)
    : this.planejamentos.contratos.filter(p => p.stSelecionado && this.perfilUnidade == p.coFilial);
    ...
    public  exportarExcelAtualizacaoSAP(coExercicio: number){
      ...
      openModalPlanejamento(tipoModal: string, isEditable: boolean, isCadastro : boolean, planejamento?: any, nuPlanejamentoOrcamento?:number, nuAno?:number) {
        ...
        console.log('PLANEJAMENTO ENVIADO PARA O MODAL', planejamento);
        console.log('PLANEJAMENTO ENVIADO PARA O MODAL', this.ordemTipoExercicio);
  ...
  async updateRelatorio(valor: string | null, op: number): Promise<void> {
   ...
   public async obterPlanejamentosOrc(): Promise<void> {
      this.loading = true;
      try {
        const response = await this.apiService.get<ApiResponse<Gcpvw051VisaoContratosPlanejamentoOrcamentarioResponse>>
        (`v1/PlanejamentoOrcamentarioV/listar-itens-planejados`, this.filtroRegistros);
        ...
        this.planejamentos.contratos = response?.data?.contratos.map(p => ({...p,stSelecionado: false }));
        this.planejamentos.listaContrato = ['SEM CONTRATOS', ...(response?.data?.listaContrato ?? [])];
        this.planejamentos.listaUnidadeDemandante = response?.data?.listaUnidadeDemandante ?? [];
